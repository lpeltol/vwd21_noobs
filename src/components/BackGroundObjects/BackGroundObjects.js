export default class BGOHandler {
  ...
  static drawObjects(ctx) {
    ...
    drawBackground(this.size, ctx, (this.skyObject.color === "yellow") ? "skyblue" : "#001C39", (this.skyObject.color === "yellow") ? "green" : "#001A00");
    ...
    this.objects.forEach((object) => {
      ...
      (this.skyObject.color === "yellow") ? object.color = "white" : object.color = "lightgray";
      object.draw(ctx);
      object.update();
    });
  }
...
class CelestialObject {
  ...
  update() {
    ...
    this.x = (1 - this.t) * (1 - this.t) * this.size * 0 + 2 * (1 - this.t) * this.t * this.size * 0.5 + this.t * this.t * this.size;
    this.y = (1 - this.t) * (1 - this.t) * this.size / 2 + 2 * (1 - this.t) * this.t * this.size * 0.0 + this.t * this.t * this.size / 2;

    if (this.x > this.radius * 2 * this.size + this.size) {
      this.color = (this.color == "gray") ? this.color = "yellow" : this.color = "gray";
